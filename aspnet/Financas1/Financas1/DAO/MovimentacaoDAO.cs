...
namespace Financas1.DAO
{
    public class MovimentacaoDAO
    {
        ...
        public IList<Movimentacao> Busca(decimal? valorMinimo, decimal? valorMaximo, DateTime? dataMinima, DateTime? dataMaxima, 
            Tipo? tipo, int? usuarioId)
        {
            ...
            {
                busca = busca.Where(m => m.Date >= dataMinima);
            }
            if (dataMaxima.HasValue)
            {
                busca = busca.Where(m => m.Date <= dataMaxima);
            }
            if (tipo.HasValue)
            {
